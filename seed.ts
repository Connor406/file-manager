// This file has to stay at root of project! Will cause bug on localBucket otherwise
import { PrismaClient } from "@prisma/client"
import { promises as fs } from "fs"
import { join } from "path"
import { saveFile } from "./bucket"
...
export async function seed(): Promise<void> {
  ...
  try {
    const existingRootDirectory = await client.directory.findFirst({ where: { name: "root" } })
    const rootDirectory =
      existingRootDirectory ?? (await client.directory.create({ data: { name: "root" } }))

    const filesDir = await fs.readdir(seedFilesPath)
    const files = filesDir.filter(file => file !== ".DS_Store" && file !== ".git")

    const subDir1 = await createDirectory(client, "Sub-Directory 1", rootDirectory.id)
    const subDir2 = await createDirectory(client, "Sub-Directory 2", rootDirectory.id)
    const subSubDir1 = await createDirectory(client, "Sub-Sub-Directory 1", subDir1.id)
    const subSubDir2 = await createDirectory(client, "Sub-Sub-Directory 2", subDir2.id)
