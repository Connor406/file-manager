...
export interface Sort {
  field: keyof Pick<DirectoryContentsResult, "name" | "size" | "createdAt" | "updatedAt">
  direction?: "ASC" | "DESC"
}
...
type RawResult = Array<Omit<DirectoryContentsResult, "type"> & { type: "1" | "2" }>
...
export async function getDirectoryContents(
  ...
  const [files, directories] = await client.$transaction([
    client.file.findMany({
      ...
      include: {
        versions: {
          // returns one version from each file => most recent
          distinct: ["fileId"],
...
export async function renameDirectory(
  ...
  if (name.toLowerCase() === "root") throw new Error('Directory name "root" is reserved')
...
export async function moveDirectory(
  ...
  if (!thisDirectory) throw new Error("Invalid directory")
  ...
  const descendantAncestorUpdates = [
    ...childFilesOfThisDirectory.map(file => {
      const updatedAncestors = [...destinationAncestors, destinationDirectory.id, thisDirectory.id]
    ...
    ...descendentFilesOfThisDirectory.map(file => {
      const updatedAncestors = [
        ...new Set([
          //  the Set de-duplicates this array
    ...
    ...descendantDirectoriesOfThisDirectory.map(directory => {
      const updatedAncestors = [
        ...new Set([
  ...
  const childDirectoryAncestorUpdates = client.directory.updateMany({
    where: { parentId: thisDirectory.id },
    data: { ancestors: [...destinationAncestors, destinationDirectory.id, thisDirectory.id] },
  })

  await client.$transaction([
    ...descendantAncestorUpdates,
